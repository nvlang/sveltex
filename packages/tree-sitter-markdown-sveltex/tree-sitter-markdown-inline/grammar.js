...
module.exports = grammar(add_inline_rules({
    // SVELTEX FORK: renamed from `markdown_inline` to avoid symbol/name clashes
    // with Zed's built-in markdown-inline grammar (the parser exports
    // `tree_sitter_markdown_inline_sveltex`).
    name: 'markdown_inline_sveltex',
    ...
    rules: {
        ...
        // SVELTEX FORK: a digit run may contain an intraword `_` only when it is
        // immediately followed by another digit. Upstream used `/[0-9][0-9_]*/`,
        // which greedily absorbed a *trailing* `_` into the digit token — so in
        // `_italic 1_` / `_italic1_` the closing `_` was swallowed by `1_` and
        // never got a chance to flank, blocking emphasis (the digit-before-`_`
        // bug). Requiring every `_` to be followed by a digit keeps the
        // intraword case `1_2` glued (still no emphasis, per CommonMark) while
        // leaving a delimiter-position `_` free to close emphasis. `_word_no_digit`
        // is left untouched, so `_tag_name` and the rest of the grammar are
        // unaffected.
        _digits: $ => /[0-9]([0-9]|_[0-9])*/,
