...
static bool scan(Scanner *s, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (!(s->state & STATE_MATCHING)) {
        ...
        // new blocks.
        //
        // SVELTEX FORK: CommonMark indented code blocks are disabled. Upstream
        // emits `INDENTED_CHUNK_START` here whenever a line is indented >= 4
        // columns outside a list context, which opens an `indented_code_block`
        // (the only producer of that token; see `_indented_chunk` in
        // grammar.js). Starving the token means a 4-space-indented line is left
        // to ordinary block parsing and becomes a normal paragraph, so its
        // inline markup (e.g. `_italic_`) is still parsed. List-item
        // indentation is untouched (handled by the LIST_ITEM cases in
        // `match()` and by list-marker parsing), and fenced code blocks are
        // untouched (handled by the '`'/'~' cases in the switch below).
        //
...
void *tree_sitter_markdown_sveltex_external_scanner_create(void) {
...
bool tree_sitter_markdown_sveltex_external_scanner_scan(void *payload, TSLexer *lexer,
                                                const bool *valid_symbols) {
...
unsigned tree_sitter_markdown_sveltex_external_scanner_serialize(void *payload,
                                                         char *buffer) {
    Scanner *scanner = (Scanner *)payload;
    return serialize(scanner, buffer);
}

void tree_sitter_markdown_sveltex_external_scanner_deserialize(void *payload,
                                                       const char *buffer,
                                                       unsigned length) {
    Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_markdown_sveltex_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
