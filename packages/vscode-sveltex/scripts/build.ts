...
/**
 * Locates the three language servers the extension transitively launches.
 *
 * Each server's `bin/server.js` is used as the esbuild entry: a tiny launcher
 * that imports the server's compiled core and starts it. `svelte-language-server`
 * ships built JavaScript; the two SvelTeX servers are workspace packages whose
 * `dist/` is produced by `pnpm --recursive build` before this package builds.
 *
...
function resolveServerBundles(): ServerBundle[] {
    const servers: { name: string; moduleId: string }[] = [
        ...
        },
        {
            ...
            moduleId: 'svelte-language-server/bin/server.js',
        },
    ];
