...
suite("TexHandler<'mathjax-node'>", async () => {
    const handler = await createTexHandler('mathjax-node');
    describe("createTexHandler('mathjax-node')", () => {
    ...
    describe('texHandler', () => {
        ...
        describe('configure()', () => {
            ...
            it('configures code correctly', async () => {
                await handler.configure({
                    inputConfig: { html: false, svg: true },
                });
            ...
            it('should correctly configure mathjaxNodeConfig', async () => {
                await handler.configure({
                    mathjaxNodeConfig: { displayErrors: false },
                });
                expect(
                    handler.configuration.mathjaxNodeConfig?.displayErrors,
                ).toEqual(false);
                await handler.configure({
                    mathjaxNodeConfig: { displayErrors: true },
                });
                expect(
                    handler.configuration.mathjaxNodeConfig?.displayErrors,
                ).toEqual(true);
