...
const sveltexPreprocessor = await sveltex({ codeBackend: 'highlight.js' });
...
describe('VerbatimHandler', () => {
    ...
    describe('constructor(sveltex: Sveltex)', () => {
        ...
        it("duplicate verbatim environment detection doesn't throw a false alarm in subsequent calls `configure`", async () => {
            const sp = await sveltex();
            await sp.configure({
                verbatim: {
                    CodeBlock: {
