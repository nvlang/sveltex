...
SvelTeX uses caching for advanced TeX content to reduce build times and improve
HMR support.
...
be `node_modules/.cache/@nvl/sveltex`. It will then populate it with a
...
<Step title="Update TeX component content">
    ...
    1.  Does `node_modules/.cache/@nvl/sveltex/TeX/hash_new.tex` exist?
        -   `true`: noop.
        -   `false`: create it.
    2.  Does `node_modules/.cache/@nvl/sveltex/TeX/hash_new.pdf` exist?
        -   `true`: noop.
    ...
    <AccordionGroup>
        <Accordion
            title="node_modules/.cache/@nvl/sveltex/cache.json"
        >
        ```json
        {
        ...
        <Accordion
            title="node_modules/.cache/@nvl/sveltex/"
        >
        ```gitignore
        cache.json
        TeX/hash1.tex
