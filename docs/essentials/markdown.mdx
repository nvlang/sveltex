...
```js sveltex.config.js
import { sveltex, createMarkdownHandler } from 'sveltex-preprocess';
import { Marked } from 'marked';

export const sveltexPreprocessor = await sveltex(
    'custom', // Markdown backend
    'none', // Code backend (syntax highlighting)
    'none', // TeX backend
    'none', // Advanced TeX backend
);

sveltexPreprocessor.markdownHandler = await createMarkdownHandler(
    'custom',
    {
        ...
        configure: (config, markdownHandler) => {
            ...
            // after calling the `configure` function you provide, the
