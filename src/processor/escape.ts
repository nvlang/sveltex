...
import { Sveltex } from '$processor/Sveltex.js';
import { re, splitContent } from '$processor/utils.js';
...
>(
    ...
    content: string,
    filename?: string,
    hires: boolean | 'boundary' = true,
) {
    const verbEnvsConfig = sveltex.configuration.general.verbatimEnvironments;
    const verbEnvs = Object.keys(verbEnvsConfig).flatMap((env) => {
        const description = verbEnvsConfig[env];
        if (
            description &&
            typeof description === 'object' &&
            'aliases' in description
        ) {
    ...
    /**
     * The position of the current section within the `content` string.
     */
    let offset = 0;
    ...
    sections.forEach((section) => {
        if (sectionType(section) === 'markup') {
            escapeVerbInMarkup(section, savedMatches, s, offset, [
                verbRegExp,
    ...
    const map = s.generateMap({ ...source, hires });
...
export function escapeVerbInMarkup(
    ...
    regExps.forEach((pattern) => {
        ...
        while ((match = pattern.exec(markup)) !== null) {
            ...
            if (!editedRanges.some((r) => intersection(r, range))) {
                const id = uuidv4();
                savedMatches.set(id, match[0]);
...
export function escapeMustacheTags(
    ...
    const escapedContent = s.toString();
    const map = s.generateMap({ source: filename, hires });
