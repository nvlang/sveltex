...
`<meta>`, `<link>`, `<base>`, `<noscript>` keys), the `<script>` variable
it creates (with the derived camelCase identifier — e.g. `colorScheme`
for `color-scheme`), and its entry in the `metadata` export — each with
