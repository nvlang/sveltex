...
four frontmatter-processing steps run. Each toggles independently — `head`
(the generated `<svelte:head>`), `metadata` (the `export const metadata`
module-script export), `variables` (the per-key `<script>` constants), and
`imports` (the `imports` frontmatter key) — or all at once via
`frontmatter: false`.
...
transformers regardless. Resolves #24.
